...
try:
    ...
    raise ImportError(
        "ray.tune in ray > 0.7.5 requires 'tabulate'. " "Please re-run 'pip install ray[tune]' or " "'pip install ray[rllib]'."
    )
...
@DeveloperAPI
class HydroReporterBase(ProgressReporter):
    ...
    def _progress_str(
        ...
        *sys_info: Dict,
        ...
        messages.append(self._current_best_hydrotrial(trials))

        # current_best_trial, metric = self._current_best_hydrotrial(trials)
    ...
    def _current_best_hydrotrial(self, trials: List[Trial]):
        if not trials:
            return None, None
        ...
        combined from `_best_trial_str` function.
        """
        if best_hydrotrial:
            if best_hydrotrial.is_targettrial():
                if "train_loop_config" in list(best_hydrotrial.config.keys()):
                    params = best_hydrotrial.config["train_loop_config"]
                else:
                    raise NotImplementedError
                best_trial_str = (
                    f"Current best trial: {best_hydrotrial.trial_id}(target trial) with {metric}={best_metric} and "
                    f"parameters={params}"
                )
            else:
                best_trial = best_hydrotrial.get_best_trial_inside(metric, mode)
                config = best_trial.config
                if "train_loop_config" in list(config.keys()):
                    params = config["train_loop_config"]
                else:
                    raise NotImplementedError
                best_trial_str = (
                    f"Current best trial: {best_trial.trial_id}(in {best_hydrotrial.trial_id}) with {metric}={best_metric} and "
                    f"parameters={params}"
                )
        else:
            best_trial_str = "Current best trial: None"

        return best_trial_str
...
def _best_trial_str(
    ...
    return f"Current best trial: {trial.trial_id} with {metric}={val} and " f"parameters={params}"


@PublicAPI
class HydroCLIReporter(HydroReporterBase):
...
def _max_len(value: Any, max_len: int = 20, add_addr: bool = False, wrap: bool = False) -> Any:
    ...
    if string == "train_loop_config/batch_size":
        return "bs"

    if "train_loop_config" in string:
        return string.replace("train_loop_config/", "")
...
def _get_progress_table_data(
    ...
    metric_keys = [
        k for k in metric_keys if any(unflattened_lookup(k, t.last_result, default=None) is not None for t in trials)
    ]
    ...
    if isinstance(metric_columns, Mapping):
        formatted_metric_columns = [
            _max_len(metric_columns[k], max_len=max_column_length, add_addr=False, wrap=True) for k in metric_keys
        ]
    else:
    ...
    if isinstance(parameter_columns, Mapping):
        ...
        formatted_parameter_columns = [
            _max_len(k, max_len=max_column_length, add_addr=False, wrap=True) for k in parameter_keys
        ]
    columns = ["Trial name", "status", "loc", "hydro"] + formatted_parameter_columns + formatted_metric_columns
...
def _get_trial_hydro_config(trial: Trial, config: dict) -> _Location:
    config = config["train_loop_config"]
    fusion, scaling = config.get("FUSION_N", None), config.get("SCALING_N", None)
    if fusion and scaling:
        hydro_config = f"F={abs(fusion)}, S={abs(scaling)}"
    else:
        hydro_config = "Target"
    return hydro_config
...
def _get_trial_info(trial: Trial, parameters: List[str], metrics: List[str], max_column_length: int = 20):
    ...
    hydro = _get_trial_hydro_config(trial, config)
    trial_info = [str(trial), trial.status, str(location), str(hydro)]
    trial_info += [
        _max_len(
...
class TrialProgressCallback(Callback):
    ...
    def print_result(self, trial: Trial, result: Dict, error: bool, done: bool):
        ...
        if has_verbosity(Verbosity.V3_TRIAL_DETAILS):
            ...
            if error:
                message = f"The trial {trial} errored with " f"parameters={trial.config}. " f"Error file: {error_file}"
            elif self._metric:
                message = (
                    f"Trial {trial} reported " f"{metric_name}={metric_value:.2f} " f"with parameters={trial.config}.{info}"
                )
            else:
                message = f"Trial {trial} reported " f"{print_result_str} " f"with parameters={trial.config}.{info}"
    ...
    def generate_trial_table(self, trials: Dict[Trial, Dict], columns: List[str]) -> str:
        ...
        return Template("trial_progress.html.j2").render(
            table=tabulate(data, tablefmt="html", headers=["Trial name"] + columns, showindex=False)
        )
...
def _detect_reporter(hydro_enable: bool, **kwargs) -> ProgressReporter:
    ...
    """
    if hydro_enable:
        progress_reporter = HydroCLIReporter(**kwargs)
    elif IS_NOTEBOOK:
...
def _detect_progress_metrics(trainable: Optional[Union["Trainable", Callable]]) -> Optional[List[str]]:
...
def _prepare_progress_reporter_for_ray_client(
    ...
    progress_reporter = progress_reporter or _detect_reporter(hydro_enable)
