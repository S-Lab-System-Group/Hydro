...
def prepare_model(
    ...
    if not wrap_ddp:
        warnings.warn(
            "The `wrap_ddp` argument is deprecated as of Ray 2.1. Use the " "`parallel_strategy` argument instead.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    if ddp_kwargs:
        warnings.warn(
            "The `ddp_kwargs` argument is deprecated as of Ray 2.1. Use the " "`parallel_strategy_kwargs` arg instead.",
            DeprecationWarning,
            stacklevel=2,
        )
...
@PublicAPI(stability="beta")
def accelerate(config, amp: bool = False) -> None:
    ...
    """
    try:
        set_accelerator(_TorchAccelerator(config, amp=amp))
    except RuntimeError:
        raise RuntimeError(
...
@PublicAPI(stability="beta")
def prepare_optimizer(optimizer: torch.optim.Optimizer, params: Any, **kwargs) -> torch.optim.Optimizer:
    ...
    Returns:
        A wrapped optimizer.
    """
    return get_accelerator(_TorchAccelerator).prepare_optimizer(optimizer, params, **kwargs)


@PublicAPI(stability="beta")
def prepare_scheduler(scheduler: _LRScheduler, optimizer: Optimizer, **kwargs) -> _LRScheduler:
    """Wraps scheduler to support model fusion.
    ...
    Returns:
        A wrapped optimizer.
    """
    return get_accelerator(_TorchAccelerator).prepare_scheduler(scheduler, optimizer, **kwargs)
...
class _TorchAccelerator(Accelerator):
    ...
    def __init__(self, config: dict, amp: bool = False):
        self.config = config
        ...
        self.previous_meta_data = self.config.get(PREVIOUS_META, None)

    def model_scaling(self, model):
        scaled_model = scale_model(model, scaling_ratio=self.scaling_num)
        base_shape = make_base_shapes(model, scaled_model, savefile=None)
        set_base_shapes(scaled_model, base_shape)
        logger.info(f"Model has been initialized with scaling ratio: {self.scaling_num}")
        return scaled_model

    def model_fusion(self, model):
        fused_model = fuse_model(model, self.fusion_num)
        logger.info(f"Model has been fused with fusion number: {self.fusion_num}")
        return fused_model

    def model_scaling_and_fusion(self, model):
        ...
        scaled_model = scale_model(model, scaling_ratio=self.scaling_num)
        fused_model = fuse_model(scaled_model, self.fusion_num)
    ...
    def prepare_data_loader(
        ...
        if (
            ...
            and not (hasattr(data_loader, "dataset") and isinstance(data_loader.dataset, IterableDataset))
    ...
    def prepare_optimizer(self, optimizer: Optimizer, params: Any, **kwargs) -> Optimizer:
        """Wraps optimizer to support automatic mixed precision.
        ...
        Returns:
            A wrapped optimizer.
        """
        if self.previous_meta_data is not None and "lr" in self.config.keys():
            initial_lr = self.config["lr"]
            optimizer = hydro_optimizer(optimizer, params, self.fusion_num, self.scaling_num, initial_lr=initial_lr, **kwargs)
        else:
            optimizer = hydro_optimizer(optimizer, params, self.fusion_num, self.scaling_num, **kwargs)
        return _WrappedOptimizer(optimizer, scaler=self.scaler)

    def prepare_scheduler(self, scheduler: _LRScheduler, optimizer: Optimizer, **kwargs) -> _LRScheduler:
        """Wraps optimizer to support automatic mixed precision.
        ...
        Returns:
            A wrapped optimizer.
        """
        if self.previous_meta_data is not None:
            last_epoch = self.previous_meta_data["iteration"]
        else:
            last_epoch = -1
        return hydro_lr_scheduler(scheduler, optimizer, self.fusion_num, last_epoch, **kwargs)
    ...
    def _move_to_device(self, item):
        ...
        with torch.cuda.stream(self._memcpy_stream):
            if isinstance(item, collections.abc.Mapping):
                ...
                logger.info(f"Data type {type(item)} doesn't support being moved to device.")
                item_on_device = item
